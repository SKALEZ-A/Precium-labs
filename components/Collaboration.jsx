    ...
    <Section crosses>
      <div className="max-w-[77.5rem] mx-auto px-5 md:px-10 lg:px-15 xl:max-w-[87.5rem] lg:flex">
          ...
          <ul className="max-w-[22rem] mb-10 md:mb-14">
            {collabContent.map((item) => (
              <li className="mb-3 py-3" key={item.id}>
                ...
                {item.text && (
                  <p className=" font-light text-[0.875rem] leading-6 md:text-base mt-3 text-n-4">{item.text}</p>
                )}
              </li>
            ))}
          </ul>
        ...
        <div className="lg:ml-auto xl:w-[38rem] mt-4">
          ...
          <div className="relative left-1/2 flex w-[22rem] aspect-square border border-gray-600 border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
            <div className="flex w-60 aspect-square m-auto border border-gray-600 border-n-6 rounded-full">
            ...
            <ul>
              {collabApps.map((app, index) => (
                ...
                >
                  <div
                    className={`relative -top-[1.6rem] flex w-[3.2rem] h-[3.2rem] bg-gray-300 dark:bg-[#15131D] border border-n-1/15 rounded-xl -rotate-${
                      index * 45
                    }`}
                  >
                    <Image
